...
export default function RunsInsightsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get('/memory/agent_performance'),
    ])
      .then(([logsRes, perfRes]) => {
      ...
      .catch((err) => {
        ...
        setSelected(null)
        setPerf({})
        setError(err?.response?.data?.error || err?.message || 'Failed to load logs/insights')
      })
  }, [canUseAdmin])
              ...
              <Card title="Logs">
                ...
                <Table
                  ...
                  rows={filtered.map((l) => [
                    ...
                    <button className="text-blue-600" onClick={() => loadRun(l.id)}>
                      Open
                    </button>,
                  ])}
                />
            ...
            <div className="space-y-4">
              <Card title="Agent Performance">
                <Table
                  ...
                  rows={Object.entries(perf).map(([id, p]: any) => [
                    id,
                    p.runs,
                    p.avgScore?.toFixed?.(2),
                    p.avgSeverity?.toFixed?.(2),
                    p.avgCost?.toFixed?.(4),
                  ])}
                />
              </Card>
